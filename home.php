...
<body>
    ...
    if( isset($_POST['post']) ){
        ...
        $post_user_name = $login_user['name'];
        $post_user_photo = $login_user['photo'];

        // Post image
        $file_name = $_FILES['post_featured']['name'];
        $file_name_tmp = $_FILES['post_featured']['tmp_name'];
        $unique_file_name = md5(time().rand()) .'.'. $file_name;


        if( !empty($content) ){
            insert("INSERT INTO posts (name, photo, content, featured_photo) VALUES ('$post_user_name', '$post_user_photo', '$content', '$unique_file_name')");
            move_uploaded_file($file_name_tmp , 'media/posts/' . $unique_file_name);
        }
				...
				<div class="modal-body">
					...
					<form action="" method="POST" enctype="multipart/form-data">
						...
						<div class="upload clearfix shadow-sm">
                            ...
                            <input name="post_featured" style="display: none;" type="file" id="post_image">
							<ul>
								<li><a href="#"><label for="post_image"><img src="assets/media/img/icons/p1 (2).png" alt=""></label></a></li>
